...
filepath = setwd("C:/Users/tganez/OneDrive - CRG - Centre de Regulacio Genomica/Toni CRG/Results/IMPDH2 FUCCI data/")

{
  # load the files and remove columns that are all NA
  filename_object = 
    read.csv("FUCCI IMPDH2 647 Ev3.txt", header = TRUE, sep="\t",skip=9)
...
### substract green signal from around the nucleus 
    ...
    #fuccidata$dif647=fuccidata$nuc_int_647 - fuccidata$nuc_intR_647 # just for when there is supposed to not be signal on cytoplasm
...
### center the data in the gates
  ...
  setwd("C:/Users/tganez/OneDrive - CRG - Centre de Regulacio Genomica/Toni CRG/Results/IMPDH2 Fig2A and Sup") 
  ...
  
{
 ...
 pdfname = paste("U2OS - Topbot",topbot_percent," cyt647plot_GT_topbot_density.pdf")
  ggsave(pdfname, plot = cyt647plot_GT_topbot_density2, width = 40, height = 15, dpi = 150, units = "cm")

 pdfname = paste("U2OS - Topbot",topbot_percent," nuc647plot_GT_topbot_density.pdf")
  ggsave(pdfname, plot = nuc647plot_GT_topbot_density2, width = 40, height = 15, dpi = 150, units = "cm")
}
...
fuccidata$Phase_Group = factor(fuccidata$Phase_Group,levels =c("G0","G1","S","G2M","M"))
...
my_comp <- list(c("G1", "S"), 
    ...
    set.seed(123)
    boxplot_all <- ggplot(na.omit(fuccidata %>% 
                                    filter(Phase_Group %in% c("G1", "S", "G2M"))
                                  ), aes(x = Phase_Group, y = log2(nuc_int_647_intgr))) +
      ...
      stat_compare_means( # by default it uses Wilcoxon
        comparisons = my_comp,
    ...
    print(boxplot_all)
      ggsave(paste("Boxplot all - log2 nuc_int_647_intgr_plot_GT - G1_S_G2M.pdf"),
